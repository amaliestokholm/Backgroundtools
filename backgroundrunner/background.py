...
def get_working_paths(catalog_id,star_id,subdir,run=None):
    """
    ...
    data_dir = os.path.join(local_path + datadir)
    if run is not None:
        outputdir = os.path.join(local_path, str(run), resultdir)
    else:
        outputdir = os.path.join(local_path, resultdir)
    star_dir = outputdir + catalog_id + star_id + '/'
...
def set_background_priors(catalog_id,star_id,numax,model_name,dir_flag=0,numaxgaussianprior=True):
    """
    ...
    :param numaxgaussianprior: a flag determining whether to use a uniform prior (False) or Gaussian prior (True) for numax
    :type numaxgaussianprior: bool
    ...
    upper_white_noise = 1.5 * white_noise
    ...
    if numaxgaussianprior:
        header += """
        Column #3: Type of prior
        """
        boundaries = np.reshape(np.array(boundaries),(int(len(boundaries)/2),2))
        types = np.zeros(len(boundaries),dtype=int)
        types[-2] = 1
        boundaries = np.c_[boundaries, types]
        print(boundaries)
    else:
        boundaries = np.reshape(np.array(boundaries),(int(len(boundaries)/2),2))
    np.savetxt(filename, boundaries,fmt='%.3f %.3f %d', header=header)
