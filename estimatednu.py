...
def deltanu_lightkurve(frequency, power, numax, floatfill=-99, silent=False):
    ...
    # utils.get_fwhm
    if u.Quantity(frequency[-1], u.microhertz) > u.Quantity(500.0, u.microhertz):
    ...
    x0 = int(
        (frequency[0].value / frequency_spacing)
    ...
    aacf = np.correlate(p_sel, p_sel, mode="full")[
        len(p_sel) - 1 :
    ...
    th = 0.5
    try:
        sel = (lags > deltanu_emp - th * deltanu_emp) & (
            lags < deltanu_emp + th * deltanu_emp
        )
        ...
        try:
            sel = (lags > deltanu_emp - 2 * deltanu_emp) & (
                lags < deltanu_emp + 2 * deltanu_emp
            )
...
def deltanu_famed(
    ...
    crush_coeff = (np.sum((frequency[tmp_crush] - numax) ** 4) / np.sum(tmp_crush)) / (
        sigma**4
    )
    if crush_coeff < 10:
    ...
    try:
        coeff2, _ = curve_fit(gauss_nterm3, x2, y2, p0=estimates)
    except RuntimeError:
        return runtimeerror, runtimeerror, best_dnu2
...
def extract_parameters(star_id, bgrun, returnpsd=False, corot=True):
    # This part is for the specific setup of the CoRoT targets
    if corot:
        if bgrun in ["05", "06"]:
            datadir = "./data"
        elif bgrun in ["03", "04"]:
    ...
    medianpar, lowerpar, upperpar = np.loadtxt(sumf, unpack=True, usecols=(1, 4, 5))
    n_par = len(medianpar)
...
if __name__ == "__main__":
    ...
    dnu_famed, error_dnu_famed, dnu_smth = deltanu_famed(*e)
